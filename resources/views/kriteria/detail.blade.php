...
@section('content')
        ...
        <div class="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    @if ($totalBobot == 100)
                        <span
                            class="inline-flex items-center mt-2 px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                    ...
                    @elseif($totalBobot > 100)
                        <span
                            class="inline-flex items-center mt-2 px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">
                            Melebihi 100%
                        </span>
                    @else
                        <span
                    ...
                    @forelse($subKriteria as $index => $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                ...
                                @else
                                    @php
                                        $countOptions = \App\Models\SistemKriteria::where('id_parent', $item->id)
                                            ->where('level', 3)
                                            ->count();
                                    @endphp
                            ...
                            <td class="px-6 py-4 text-center text-sm font-medium">
                                <div class="flex items-center justify-center gap-1.5 whitespace-nowrap">
                                    ...
                                    @if ($item->tipe_input === 'dropdown')
                                        <button type="button"
                                            onclick="openDropdownOptionsModal({{ $kriteria->id }}, {{ $item->id }}, '{{ $item->nama_kriteria }}')"
                                            class="inline-flex items-center px-2.5 py-1.5 bg-purple-600 text-white text-xs font-medium rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-150 cursor-pointer">
                                    ...
                                    {{-- Button Edit --}}
                                    <button type="button"
                                        onclick="editSubKriteria({{ $kriteria->id }}, {{ $item->id }})"
                                    ...
                                    <form
                                        action="{{ route('kriteria.sub-kriteria.toggle-status', [$kriteria->id, $item->id]) }}"
                                        method="POST" class="inline-block">
                                        @csrf
                                        <button type="submit"
                                            class="inline-flex items-center px-2.5 py-1.5 {{ $item->is_active ? 'bg-gray-600 hover:bg-gray-700 focus:ring-gray-500' : 'bg-green-600 hover:bg-green-700 focus:ring-green-500' }} text-white text-xs font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-150 cursor-pointer">
                                            @if ($item->is_active)
                                                <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    {{-- Button Hapus --}}
                                    <button type="button"
                                        onclick="confirmDelete({{ $kriteria->id }}, {{ $item->id }}, '{{ $item->nama_kriteria }}')"
    ...
    <div id="modal-tambah-sub-kriteria" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-xl shadow-2xl">
                ...
                {{-- Modal Body --}}
                <form action="{{ route('kriteria.sub-kriteria.store', $kriteria->id) }}" method="POST"
                    class="p-6 space-y-4" id="form-tambah-sub-kriteria"
                    onsubmit="return validateSubKriteriaForm('tambah')">
                    @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="nilai_max" id="nilai_max_tambah" step="0.01"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
                                    placeholder="Contoh: 22" oninput="clearFieldError(this)">
                            </div>
                        </div>
                    ...
                    <div id="dropdown_options_sub_tambah"
    ...
    <div id="modal-edit-sub-kriteria" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-xl shadow-2xl">
                ...
                <form id="form-edit-sub-kriteria" method="POST" class="p-6 space-y-4"
                    onsubmit="return validateSubKriteriaForm('edit')">
                    @csrf
                    @method('PUT')
                    ...
                    {{-- Conditional Dropdown Options Fields --}}
                    <div id="dropdown_options_sub_edit"
                        class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 hidden">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="text-sm font-medium text-gray-700">Dropdown Options</h4>
    ...
    <div id="modal-hapus-sub-kriteria" tabindex="-1" aria-hidden="true"
        class="hidden fixed inset-0 z-50 items-center justify-center overflow-y-auto overflow-x-hidden">
    ...
    <div id="modal-dropdown-options" tabindex="-1" aria-hidden="true"
        class="hidden fixed inset-0 z-50 items-center justify-center overflow-y-auto overflow-x-hidden">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl max-h-[90vh] flex flex-col">
                ...
                <div class="flex items-center justify-between p-5 border-b border-gray-200 rounded-t flex-shrink-0">
                    ...
                    <button type="button" onclick="closeModal('modal-dropdown-options')"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center transition-colors cursor-pointer">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                ...
                <div class="p-6 overflow-y-auto flex-1">
                    ...
                    <div class="mb-6">
                        <button id="btn-show-form" type="button" onclick="showTambahOptionForm()"
                            class="w-full px-4 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-medium rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer">
                        ...
                        <div id="form-tambah-option" class="hidden mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <form id="form-tambah-option-inner"
                                action="{{ route('kriteria.dropdown-options.store', [$kriteria->id, 0]) }}"
                                ...
                                <div>
                                    <label for="deskripsi_option_tambah"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="nilai_tetap" id="nilai_option_tambah" required
                                        step="0.01" min="0"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                                        placeholder="Contoh: 5, 4, 3">
                                    <p class="mt-1 text-xs text-gray-500">Nilai numerik yang akan digunakan untuk
                                        perhitungan TOPSIS</p>
                                </div>

                                <div class="flex items-center gap-2 pt-2">
                                    <button type="submit"
                                        class="flex-1 px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors cursor-pointer">
                                    ...
                                    <button type="button" onclick="hideTambahOptionForm()"
                                        class="flex-1 px-4 py-2 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                                        Batal
                                    </button>
                                </div>
                            </form>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">No
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Nama
                                            Option</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">
                                            Deskripsi</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Nilai
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">
                                            Status</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-center text-xs font-semibold text-gray-700 uppercase">
                                            Aksi</th>
                                    </tr>
                                </thead>
                ...
                <div class="flex items-center justify-end p-5 border-t border-gray-200 rounded-b flex-shrink-0">
                    <button type="button" onclick="closeModal('modal-dropdown-options')"
                        class="px-5 py-2.5 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors cursor-pointer">
                        Tutup
                    </button>
                </div>
            </div>
    ...
    {{-- Modal Edit Option --}}
    <div id="modal-edit-option" tabindex="-1" aria-hidden="true"
        class="hidden fixed inset-0 z-50 items-center justify-center overflow-y-auto overflow-x-hidden">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-xl shadow-2xl">
                {{-- Modal Header --}}
                ...
                <form id="form-edit-option-inner" method="POST" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="nilai_tetap" id="nilai_option_edit" required step="0.01"
                            min="0"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Contoh: 5, 4, 3">
                        <p class="mt-1 text-xs text-gray-500">Nilai numerik yang akan digunakan untuk perhitungan TOPSIS
                        </p>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-4 border-t border-gray-200">
                        <button type="submit"
                            class="flex-1 px-5 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-150 cursor-pointer">
                            Simpan Perubahan
                        </button>
                        <button type="button" onclick="closeModal('modal-edit-option')"
                            class="flex-1 px-5 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-all duration-150 cursor-pointer">
    ...
    {{-- Modal Delete Option --}}
    <div id="modal-delete-option" tabindex="-1" aria-hidden="true"
        class="hidden fixed inset-0 z-50 items-center justify-center overflow-y-auto overflow-x-hidden">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-xl shadow-2xl">
                <div class="p-6 text-center">
                    ...
                    <form id="form-delete-option" method="POST" class="inline-block w-full">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="submit"
                                class="flex-1 px-5 py-2.5 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-all cursor-pointer">
                                Ya, Hapus
                            </button>
                            <button type="button" onclick="closeModal('modal-delete-option')"
                                class="flex-1 px-5 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all cursor-pointer">
                                Batal
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function clearValidationErrors(mode) {
            ...
            const form = mode === 'tambah' ? document.getElementById('form-tambah-sub-kriteria') : document.getElementById(
        ...
        // Validate Sub-Kriteria Form before submission
        function validateSubKriteriaForm(mode) {
            ...
            if (tipeInput === 'dropdown') {
                const options = container.querySelectorAll('[id^="option_item_sub_"]');
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('form-edit-sub-kriteria').action =
                            `/kriteria/${kriteriaId}/sub-kriteria/${subKriteria.id}`;
        ...
        function loadDropdownOptions(kriteriaId, subKriteriaId) {
            ...
            tbody.innerHTML =
                '<tr><td colspan="6" class="px-6 py-8 text-center text-gray-500"><svg class="animate-spin h-8 w-8 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p class="mt-2">Memuat data...</p></td></tr>';
            ...
            fetch(`/kriteria/${kriteriaId}/sub-kriteria/${subKriteriaId}/options`)
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-1.5">
                                            <button type="button" onclick="editDropdownOption(${kriteriaId}, ${subKriteriaId}, ${option.id})"
                                                class="inline-flex items-center px-2.5 py-1.5 bg-blue-600 text-white text-xs font-medium rounded hover:bg-blue-700 transition-colors cursor-pointer">
                                            ...
                                            <form action="/kriteria/${kriteriaId}/sub-kriteria/${subKriteriaId}/options/${option.id}/toggle-status" method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit"
                                                    class="cursor-pointer inline-flex items-center px-2.5 py-1.5 ${option.is_active ? 'bg-gray-600 hover:bg-gray-700' : 'bg-green-600 hover:bg-green-700'} text-white text-xs font-medium rounded transition-colors">
                                                    <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <button type="button" onclick="confirmDeleteOption(${kriteriaId}, ${subKriteriaId}, ${option.id}, '${option.nama_kriteria}')"
                                                class="inline-flex items-center px-2.5 py-1.5 bg-red-600 text-white text-xs font-medium rounded hover:bg-red-700 transition-colors cursor-pointer">
                                                <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                .catch(error => {
                    ...
                    tbody.innerHTML =
                        '<tr><td colspan="6" class="px-6 py-8 text-center text-red-500">Gagal memuat data. Silakan coba lagi.</td></tr>';
                });
        }
            ...
            fetch(`/kriteria/${kriteriaId}/sub-kriteria/${subKriteriaId}/options/${optionId}/edit`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Set form action
                        document.getElementById('form-edit-option-inner').action =
                            `/kriteria/${kriteriaId}/sub-kriteria/${subKriteriaId}/options/${optionId}`;
        ...
        function confirmDeleteOption(kriteriaId, subKriteriaId, optionId, optionName) {
            ...
            document.getElementById('form-delete-option').action =
                `/kriteria/${kriteriaId}/sub-kriteria/${subKriteriaId}/options/${optionId}`;
