...
@section('content')
    <div class="container mx-auto px-4 py-6">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('penilaian.index') }}"
                            class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 cursor-pointer">Penilaian
                            Karyawan</a>
                    </div>
                </li>
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('penilaian.index') }}"
                    class="inline-flex items-center px-4 py-2 bg-gray-200 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 transition-colors duration-150 cursor-pointer">
                ...
                <a href="{{ route('penilaian.edit', [$karyawan->id, $bulan, $tahun]) }}"
                    class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors duration-150 cursor-pointer">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button type="button" onclick="showDeleteModal()"
                    class="inline-flex items-center px-4 py-2 bg-red-600 text-white text-sm font-semibold rounded-lg hover:bg-red-700 transition-colors duration-150 cursor-pointer">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                @if ($item->subKriteria)
                                                    @if ($item->subKriteria->tipe_input === 'angka')
                                                        <span
                                                            class="inline-flex items-center px-2.5 py-0.5 bg-blue-100 text-blue-800 text-xs font-medium rounded">
                                                    ...
                                                    @elseif($item->subKriteria->tipe_input === 'rating')
                                                        <span
                                                            class="inline-flex items-center px-2.5 py-0.5 bg-yellow-100 text-yellow-800 text-xs font-medium rounded">
                                                            <svg class="w-3 h-3 mr-1" fill="currentColor"
                                                                viewBox="0 0 20 20">
                                                                <path
                                                    ...
                                                    @elseif($item->subKriteria->tipe_input === 'dropdown')
                                                        <span
                                                            class="inline-flex items-center px-2.5 py-0.5 bg-purple-100 text-purple-800 text-xs font-medium rounded">
                                                            <svg class="w-3 h-3 mr-1" fill="currentColor"
                                                                viewBox="0 0 20 20">
                                                                <path fill-rule="evenodd"
            ...
            <div
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-600">
                                Apakah Anda yakin ingin menghapus semua penilaian untuk
                                <strong>{{ $karyawan->nama }}</strong> periode <strong>{{ $periodeLabel }}</strong>?
                            </p>
                ...
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                    <form action="{{ route('penilaian.destroy', [$karyawan->id, $bulan, $tahun]) }}" method="POST"
                        class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="inline-flex justify-center w-full px-4 py-2 text-base font-semibold text-white bg-red-600 border border-transparent rounded-lg shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm cursor-pointer">
                            Ya, Hapus
                        </button>
                    </form>
                    <button type="button" onclick="hideDeleteModal()"
                        class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:w-auto sm:text-sm cursor-pointer">
                        Batal
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className =
                `flex items-center w-full max-w-sm p-4 text-white ${bgColor} rounded-lg shadow-lg transform transition-all duration-300 ease-in-out translate-x-0 opacity-100`;
            toast.innerHTML = `
            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg bg-white bg-opacity-20">
                ${icon}
            </div>
            <div class="ml-3 text-sm font-medium">${message}</div>
            <button type="button" class="ml-auto -mx-1.5 -my-1.5 rounded-lg p-1.5 inline-flex h-8 w-8 hover:bg-white hover:bg-opacity-20 transition-colors cursor-pointer" onclick="this.parentElement.remove()">
